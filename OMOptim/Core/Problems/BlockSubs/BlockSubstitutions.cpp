...
BlockSubstitutions::BlockSubstitutions(Project* project,ModModelPlus* model,const QDomElement & el)
{
	...
	while( !e2.isNull() )
	{
		...
		{
                        BlockSubstitution* newBlock = new BlockSubstitution(project,model,model->connections(),e2);
                        add(newBlock);
		}
