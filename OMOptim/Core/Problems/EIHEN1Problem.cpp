...
EIHEN1Problem::EIHEN1Problem(Project* project,ModClassTree* modClassTree,MOomc* moomc)
...
EIHEN1Problem::EIHEN1Problem(Project* project,ModClassTree* modClassTree,MOomc* moomc,QDomElement domProblem)
...
EIHEN1Problem::EIHEN1Problem(const EIHEN1Problem &problem)
    :EIProblem(problem)
{
     _type = Problem::EIHEN1TYPE;
     _milpHEN1 = NULL;
...
Problem* EIHEN1Problem::clone() const
{
    Problem* problem = new EIHEN1Problem(*this);
    return problem;
}

EIHEN1Problem::~EIHEN1Problem(void)
{
    if(_milpHEN1)
        delete _milpHEN1;
}
...
QDomElement EIHEN1Problem::toXmlData(QDomDocument & doc)
{
...
bool EIHEN1Problem::checkBeforeComp(QString & error)
{
    bool result = true;
...
QString EIHEN1Problem::infos()
{
    QString infos;
    infos += "EIHEN1Problem solves a Heat Exchanger Network problem. Algorithm is based on following article : \n";
...
Result* EIHEN1Problem::launch(ProblemConfig config)
{
    ...
    else
    {
        ...
        if(splitPinch)
        {
            EIMERProblem merProblem(_project,_modClassTree,_moomc);
...
bool EIHEN1Problem::canBeStoped()
{
    return true;
}

void EIHEN1Problem::stop()
{
    if(_milpHEN1)
        _milpHEN1->stop();
}
...
//void EIHEN1Problem::onStopAsked()
//{
//    if(_milpHEN1)
//        _milpHEN1->stop();
//}
