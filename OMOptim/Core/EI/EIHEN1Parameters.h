...
public :
    ...
    static void setDefaultParameters(MOParameters *parameters)
    {
        ...
        parameters->addItem(new MOParameter((int)ALLOWSEVERAL,"Allow several heat exchangers between two streams","B",true,MOParameter::BOOL));
        parameters->addItem(new MOParameter((int)ALLOWSPLITS,"Allow splits","S",true,MOParameter::BOOL));
        parameters->addItem(new MOParameter((int)KMAX,"Maximum heat exhangers between two streams","S",true,MOParameter::INT,10,std::numeric_limits<int>::max()));
    };
};
