...
#include "OMTableDialog.h"
#include "AlgoParameter.h"
#include "MOTableView.h"

#include <QtGui/QScrollBar>

OMTableDialog::OMTableDialog(QAbstractItemModel *model)
{
	model = _model;
	QGridLayout *newLayout = buildLayout();
	this->setLayout(newLayout);
	
}

QGridLayout* OMTableDialog::buildLayout()
{
	//Adding Layout
	...
	// localParameters = new MOVector<AlgoParameter>;
	// localParameters->cloneFromOtherVector(eaConfig->parameters);

	// GuiTools::ModelToView(localParameters,table);

	// // resizing window
	// int count = eaConfig->parameters->columnCount(QModelIndex());
	// int w=0;
	...
	// count = eaConfig->parameters->rowCount(QModelIndex());
	// int h=0;
    // for (int i = 0; i < count; i++)
		// h += table->rowHeight(i);
...
void OMTableDialog::pushedOk()
{

	// eaConfig->parameters->cloneFromOtherVector(localParameters);
...
void OMTableDialog::pushedCancel()
{
