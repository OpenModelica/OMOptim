        ...
        MO2ColTab(project->name(),problem,false,parent)
{
    ...
    //problem given in argument could be either a EITarget,EIMER or EIHEN
    ...
    if(dynamic_cast<EITarget*>(problem))
     ...
     // create new ProblemMER or copy from problem
    if(dynamic_cast<EIMER*>(problem))
        _problemMER = new EIMER(*dynamic_cast<EIMER*>(problem));
    else
        _problemMER = new EIMER(_project,_project->modClassTree(),_project->moomc());

    // create new ProblemHEN or copy from problem
    if(dynamic_cast<EIHEN1*>(problem))
        _problemHEN = new EIHEN1(*dynamic_cast<EIHEN1*>(problem));
    else
