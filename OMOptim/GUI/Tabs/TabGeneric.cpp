...
void TabGeneric::setWidgets(QWidgetList _widgets,QStringList _titles) 
{
	widgets = _widgets;
	widgetsTitles = _titles;
	


	for(int i=0;i<widgetsTitles.size();i++)
	{
		ui->mainLayout->addWidget(_widgets.at(i));
		ui->comboDisp->addItem(widgetsTitles.at(i),i);
	}
...
{
	int index = ui->comboDisp->itemData(view).toInt();
